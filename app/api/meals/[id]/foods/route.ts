  ...
  try {
    ...
    if (dbValidation.success) {
      ...
      if (food.source === 'USER' && food.userId !== session.user.id) {
        return NextResponse.json({ error: '無權限使用此自訂食物' }, { status: 403 });
      }
    ...
    return NextResponse.json({ error: '新增食物失敗' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (!mealFoodId) {
      return NextResponse.json({ error: '缺少 mealFoodId 參數' }, { status: 400 });
    }
    ...
    if (!mealFood) {
      return NextResponse.json({ error: '找不到此食物或無權限刪除' }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: '移除食物失敗' }, { status: 500 });
  }
}
