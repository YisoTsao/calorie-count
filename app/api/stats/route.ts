  ...
  try {
    ...
    meals.forEach(
      (meal: {
        ...
        meal.foods.forEach(
          (mealFood: { calories: number; protein: number; carbs: number; fat: number }) => {
    ...
    // 6. 判斷目標達成
    const calorieGoalMet = goals
      ? Math.abs(totalCalories - goals.dailyCalorieGoal) <= goals.dailyCalorieGoal * 0.1
      : false;
    ...
    return NextResponse.json({ error: '計算每日統計失敗' }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (goals) {
      for (const day of dateMap.values()) {
        if (day.totalCalories > 0) {
          day.calorieGoalMet =
            Math.abs(day.totalCalories - goals.dailyCalorieGoal) <= goals.dailyCalorieGoal * 0.1;
        }
    ...
    const stats = Array.from(dateMap.values()).sort((a, b) => a.date.localeCompare(b.date));
...
function calculateCurrentStreak(stats: Array<{ date: string; mealCount: number }>): number {
  ...
  const dateSet = new Set(stats.filter((s) => s.mealCount > 0).map((s) => s.date));
