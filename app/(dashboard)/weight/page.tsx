  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-3 text-sm shadow-lg">
      ...
      <p className="text-purple-600">
        體重: <span className="font-bold">{data.weight.toFixed(1)} kg</span>
      </p>
      <p>
      ...
      {data.bodyFat != null && (
        <p>
          體脂率: <span className="font-bold">{data.bodyFat.toFixed(1)}%</span>
        </p>
      )}
      <p className="mt-1.5 border-t pt-1.5 text-xs text-gray-400">👆 點擊可查看詳情並編輯</p>
    </div>
  );
}
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">追蹤您的體重變化與 BMI</p>
        </div>
        <button
          onClick={() => {
            setEditingRecord(null);
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm">
        <h3 className="mb-4 text-lg font-semibold text-gray-900">查看期間</h3>
        ...
        <p className="mt-3 text-sm text-gray-500">顯示 {records.length} 筆記錄</p>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-md rounded-xl bg-white p-6">
            ...
            >
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">體重 (kg) *</label>
                <input
                  type="number"
                  step="0.1"
